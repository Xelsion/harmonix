	...
	public static function getUserInputFor(string $field_name, mixed $value, array $value_set = []): string {
		...
		if( !empty($value_set) ) {
			...
			foreach( $value_set as $v ) {
				$field .= '<option value="' . $v . '"' . (($value === $v) ? ' selected="selected"' : '') . '>' . $v . '</option>';
			}
			$field .= '</select>';
		} else if( is_bool($value) ) {
			$field .= '<select name="' . $field_name . '">';
			$field .= '<option value="true"' . (($value === true) ? ' selected="selected"' : '') . '>true</option>';
			$field .= '<option value="false"' . (($value === false) ? ' selected="selected"' : '') . '>false</option>';
			$field .= '</select>';
		} else {
			$field .= '<input type="text" name="' . $field_name . '" value="' . $value . '" />';
		}
		return $field;
	}
